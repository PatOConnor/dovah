...
def write_to_file(data, is_test=False):
    ...
    if is_test:
        filename = os.path.join(dirname, 'tests/skyrimalchemy.py')
    else:
        dirname = dirname[:-14]#chop off end of dirname to go into parent folder
    ...
    for item in data:  #this writes it in python dictionary syntax
        file.write('\t'+str(data.index(item))+':'+str(item)+',\n')
    file.write('\t}')
    file.close()
