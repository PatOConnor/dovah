...
def add_cmd(arg=None):
    ...
    if arg:
        ...
        key = catalog_lookup(basearg, 'commands')
        if key < 0:
            key = _ask_for_cmd_key()
        cmdbase = skyrim_commands[key]['BASE']
    else:
        key = _ask_for_cmd_key()
        cmdbase = skyrim_commands[key]['BASE']
    ...
    other_params = skyrim_commands[key]['PARAMS']
    param_answers = []
    if arg:
    ...
    self.cmd_list.append(__form_cmd(cmdbase, target, param_answers))


'''Saves command as a list to folder designated as skyrim directory'''
def save_cmds(filename, testing=False):
...
def load_cmds(filename, testing=False):
    if testing:
        filename ='/batches/'+filename+'.txt'
    else:
        filename = self.skypath+'/'+filename+'.txt'
